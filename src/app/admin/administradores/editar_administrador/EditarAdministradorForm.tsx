...
interface AdministradorEditFormProps {
    admins: Admins[]
    userAdmins: Usuarios[]
}

export default function EditarAdministradorForm({
    admins,
    userAdmins
}: AdministradorEditFormProps) {

    const [formErrors, setFormErrors] = useState<{ [key: string]: string[] }>({}) //Objeto responsavel por validação do formulario
    const [data, handleCriarConsultor, isPending] = useActionState(updateAdminAction, null)
    ...
    const userAdminConsultado = userAdmins.find(user => user.id == id)
    ...
    return (
        ...
        <>
            <div className="consultorDados">
                <p><strong>Nome: </strong>{userAdminConsultado?.nome} {userAdminConsultado?.sobrenome}</p>
            </div>
            <form className='consultor-form' action={handleCriarConsultor}>
                ...
                <div className='button-submit-grid' style={{ gridColumn: '1 /-1' }}>
                    {id ? <Input type="hidden" name="id" value={id.toString()} /> : null}
