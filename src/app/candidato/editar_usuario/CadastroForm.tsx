...
export function CadastroForm({ cargos, candidatos, userCandidatos }: FormProps) {
    ...
    const userCandidato = userCandidatos.find(user => user.id == id)
    const candidato = candidatos.find(candidato => candidato.user_id == id)

    const [form, setForm] = useState({
        foto_usuario: '',
        nome: userCandidato?.nome,
        sobrenome: '',
        email: '',
        celular_1: '',
        celular_2: '',
    ...
    useEffect(()=>{
        candidato && userCandidato ? setForm({
            ...form,
            nome: userCandidato.nome,
            sobrenome: userCandidato.sobrenome,
            email : userCandidato.email,
            celular_1: userCandidato.celular_1,
            celular_2: userCandidato.celular_2 || '',
            data_nascimento: userCandidato.data_nascimento,
            linkedin : userCandidato.linkedin || '',
            
            
        }) : null
    },[candidato, userCandidato, id])
